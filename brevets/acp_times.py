...
def update_time(control, limit_type_number): # Limit type can either be a one or a two indicating
   ...
   if control == 0 and limit_type_number == 1:
      return time + 1
   while(not complete):

   # Determine if this is the correct section we will do final calculations with
      if (traveled + control) <= BREVET_TABLE[section][0]:
         time += control / BREVET_TABLE[section][limit_type_number]
         complete = True

      # Update time to include current section
      else:
         prev = BREVET_TABLE[section][0] - traveled
         time += prev/BREVET_TABLE[section][limit_type_number]
         section += 1
         control -= prev
         traveled += prev
   return time